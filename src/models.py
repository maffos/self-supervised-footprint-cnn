    ...
    def __init__(self, in_channels,out_channels,
        ...
        if input_downsample:
            self.conv_layer = nn.Sequential(
                nn.Conv1d(self.in_channels, self.hidden_dims[0], kernel_size=3, stride=1, padding=0, bias=self.bias),
                get_norm_layer(norm, self.hidden_dims[0], num_groups), self.activation)
        else:
...
class GraphTriangleEncoder(MessagePassing):
    def __init__(self, in_channels, out_channels, hidden_dims, activation, norm = 'graph', mlp_norm = 'graph', graph_operator = 'gcn', projection = 'mlp', conv_params = {}, dropout = 0, num_groups = None):
...
class GraphTriangleBlock(MessagePassing):
    def __init__(self, in_channels, out_channels, activation, norm='graph', mlp_norm = 'graph', graph_operator='gcn', bottleneck = True, conv_params = {}, num_groups = 0, dropout = 0):
